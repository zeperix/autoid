...
parser.add_argument("-lang", help="Output language", default="vi_vn")
...
if args.lang == "vi_vn" or args.lang == "":
    from lang import vi_vn as lang
elif args.lang == "en_us":
    from lang import en_us as lang
else:
    logger.error("Ngôn ngữ không được hỗ trợ | Language not supported")
    exit(1)
lang_text = lang()
debug = args.debug
...
class Config:
    def __init__(self, config_result):
        ...
        if debug:
            logger.info("Đã bật chế độ gỡ lỗi | Debug mode enabled")
            self.headless = False
...
class ID:
    ...
    def login_appleid(self):
        logger.info("Đang đăng nhập AppleID | Start logging in AppleID")
        try:
